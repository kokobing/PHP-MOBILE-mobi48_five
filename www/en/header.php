...
<div id="headertop">
      <div id="headerlogo"><a href="/en/"><img src="/upload/layout/<?=$logopic?>"></a></div>
...
<div id="qrcode" data-role="popup" class="ui-content" style="max-width:280px">
        ...
        <ul data-role="listview" >
            ...
            <li data-icon="arrow-r"><a href="/en/" style="font-size:14px;">Home</a></li>
                <?php
	...
	for($i=0;$i<sizeof($getallpagesetinfo);$i++){
		...
		if($getallpagesetinfo[$i][id]==23){$flienameurl='/en/news/newsindex.php';}//新闻资讯
		if($getallpagesetinfo[$i][id]==22){$flienameurl='/en/product/productindex.php';}//产品家族
		if($getallpagesetinfo[$i][id]==28){$flienameurl='/en/product/producthot.php';}//热卖手机
	?>    	
       <li data-icon="arrow-r"><a href="<?=$flienameurl;?>" style="font-size:14px;"><?=$getallpagesetinfo[$i][title]?></a></li>
     <? }?>  

            <li data-icon="delete"><a href="#" data-rel="close" style="font-size:14px;">Close</a></li>
