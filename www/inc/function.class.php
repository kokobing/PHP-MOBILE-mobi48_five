...
if($_SERVER["REMOTE_ADDR"]!='127.0.0.1' && $_SERVER["REMOTE_ADDR"]!='192.168.1.33'){
/*IS MOBI*/
...
//取出指定条数指定语言的 指定条件产品    返回变量数组 引用时需要赋值 再输出
function getconnewproduct($pnum,$lang,$ordertype,$conditions){
global	 $objDB;//函数调用 声明全局变量
$strSQL = "select a.*,b.lang,b.fatherid,b.id_proddir as pdir2 from productinfo as a left join productdir as b on a.id_proddir=b.id_proddir   where a.dele=1 and b.lang=$lang and a.tag='".$conditions."' order by a.$ordertype desc limit $pnum" ;
$objDB->Execute($strSQL);
$arrconnewproduct=$objDB->GetRows();
return $arrconnewproduct;
}
