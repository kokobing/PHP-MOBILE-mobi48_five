...
<div data-role="collapsible-set" data-theme="c" data-content-theme="d" data-inset="true" style=" margin:15px;">
    ...
    <?php
    $getallpagesetinfo=getallpagesetinfo(4,'2','title,id_pageset as id,intro');//取所有版面4俄文 （2只显示首页 1所有页）  0关闭 3只在内页
	for($i=0;$i<sizeof($getallpagesetinfo);$i++){
	?>    	

 
    <div data-role="collapsible" <?php if($getallpagesetinfo[$i][id]==45){?>data-collapsed="false" <?php }?>>
    <h2><?=$getallpagesetinfo[$i][title]?></h2>

    <?php if($getallpagesetinfo[$i][id]==46){?>
            <ul data-role="listview">
            <?php 
			$newproducts=getconnewproduct('5','4','ordernum','home');//前5条 4俄文
			for($j=0;$j<sizeof($newproducts);$j++){?>
                <li><a href="/ru/product/productview.php/<?=$newproducts[$j][pdir2]?>/<?=$newproducts[$j][id_prodinfo]?>/.html">
    ...
    <?php if($getallpagesetinfo[$i][id]==45){
	$getproductdir2=getproductdir2(29);
    ?>
        <ul data-role="listview" data-filter-theme="c" data-divider-theme="d" data-filter="true">
        <?php for($j=0;$j<sizeof($getproductdir2);$j++){?>
            <li><a href="/ru/product/productlist.php/<?=$getproductdir2[$j][id]?>/.html"><?=$getproductdir2[$j][name]?></a></li>
        <?php }?>  
        </ul>
    <?php }?>  


    <?php if($getallpagesetinfo[$i][id]==44){?>
        <ul data-role="listview" data-theme="d" data-divider-theme="d">
        
        <?php for($j=0;$j<sizeof($getnewsdir2);$j++){?>
            <li>
            <a href="/ru/news/newslist.php/<?=$getnewsdir2[$j][id]?>/.html">
                <h3><?=$getnewsdir2[$j][name]?></h3>
                <p><?=$getnewsdir2[$j][intro]?></p>
                <p class="ui-li-aside">Total <strong><?php echo getnewsnum(4,$getnewsdir2[$j][id])?></strong> news</p>
            </a>
            </li>
        <?php }?>  
    ...
    <?php if($getallpagesetinfo[$i][id]==43){?>
        <ul data-role="listview" data-filter-theme="c" data-divider-theme="d" >
			<?php
            $getallpagesetinfo=getallpagesetinfo(4,'3','title,id_pageset as id,intro');//取所有版面4俄文 （2只显示首页 1所有页）  0关闭 3只在内页
            for($i=0;$i<sizeof($getallpagesetinfo);$i++){
