...
<div id="contentbox">
	  ...
	  #feedback .ui-body-a, .ui-overlay-a {color:#404040 /*{a-body-color}*/;background: transparent none;}
        ...
        <div id="feedback">
          <div class="feedbackline">nombre *：&nbsp;&nbsp;&nbsp;&nbsp;<input id="name" type="text"  ></div>
          <div class="feedbackline">teléfono：&nbsp;&nbsp;&nbsp;&nbsp;<input id="tel" type="text" ></div>
          <div class="feedbackline">Email *：&nbsp;&nbsp;&nbsp;&nbsp;<input id="email" type="text"  ></div>
          <div class="feedbackline">mensaje *：&nbsp;&nbsp;&nbsp;&nbsp;<textarea id="content"  ></textarea></div>
          <div id="backmenu"><a onClick="sendmsg()"  href="javascript:void(0);">enviar</a></div>
...
function sendmsg() {
	                    if($.trim($("#name").val())==''){alert('Por favor, introduzca su nombre');return false;}
						if($.trim($("#email").val())=='' || !$("#email").val().match( /^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/ )){alert('Por favor, introduce una dirección de email válida');return false;}
						if($.trim($("#content").val())==''){alert('Introduzca su mensaje');return false;}	
						
						
						$.post('/sp/about/ajax_feedback.php',{tel:$("#tel").val(),content:$("#content").val(),name:$("#name").val(),email:$("#email").val()},function(data) {
