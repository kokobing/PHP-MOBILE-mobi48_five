...
<div id="headertop">
      <div id="headerlogo"><a href="/sp/"><img src="/upload/layout/<?=$logopic?>"></a></div>
...
<div id="qrcode" data-role="popup" class="ui-content" style="max-width:280px">
        ...
        <ul data-role="listview" >
            ...
            <li data-icon="arrow-r"><a href="/sp/" style="font-size:14px;">casa</a></li>
                <?php
    $getallpagesetinfo=getallpagesetinfo(3,'2','title,id_pageset as id');//取所有版面3西班牙语 （2只显示首页 1所有页）  0关闭 3只在内页
	for($i=0;$i<sizeof($getallpagesetinfo);$i++){
		$flienameurl='/sp/about/about.php/'.$getallpagesetinfo[$i][id].'/.html';//默认关于我们
		if($getallpagesetinfo[$i][id]==38){$flienameurl='/sp/news/newsindex.php';}//新闻资讯
		if($getallpagesetinfo[$i][id]==39){$flienameurl='/sp/product/productindex.php';}//产品家族
		if($getallpagesetinfo[$i][id]==40){$flienameurl='/sp/product/producthot.php';}//热卖手机
	?>    	
       <li data-icon="arrow-r"><a href="<?=$flienameurl;?>" style="font-size:14px;"><?=$getallpagesetinfo[$i][title]?></a></li>
     <? }?>  

            <li data-icon="delete"><a href="#" data-rel="close" style="font-size:14px;">cerca</a></li>
