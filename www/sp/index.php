...
<div data-role="collapsible-set" data-theme="c" data-content-theme="d" data-inset="true" style=" margin:15px;">
    ...
    <?php
    $getallpagesetinfo=getallpagesetinfo(3,'2','title,id_pageset as id,intro');//取所有版面3西班牙语 （2只显示首页 1所有页）  0关闭 3只在内页
	for($i=0;$i<sizeof($getallpagesetinfo);$i++){
	?>    	

 
    <div data-role="collapsible" <?php if($getallpagesetinfo[$i][id]==39){?>data-collapsed="false" <?php }?>>
    <h2><?=$getallpagesetinfo[$i][title]?></h2>

    <?php if($getallpagesetinfo[$i][id]==40){?>
            <ul data-role="listview">
            <?php 
			$newproducts=getconnewproduct('5','3','ordernum','home');//前5条 3西班牙语
			for($j=0;$j<sizeof($newproducts);$j++){?>
                <li><a href="/sp/product/productview.php/<?=$newproducts[$j][pdir2]?>/<?=$newproducts[$j][id_prodinfo]?>/.html">
    ...
    <?php if($getallpagesetinfo[$i][id]==39){
    ...
    ?>
        ...
        <?php for($j=0;$j<sizeof($getproductdir2);$j++){?>
            <li><a href="/sp/product/productlist.php/<?=$getproductdir2[$j][id]?>/.html"><?=$getproductdir2[$j][name]?></a></li>
        <?php }?>  
        </ul>
    <?php }?>  


    <?php if($getallpagesetinfo[$i][id]==38){?>
        <ul data-role="listview" data-theme="d" data-divider-theme="d">
        
        <?php for($j=0;$j<sizeof($getnewsdir2);$j++){?>
            <li>
            <a href="/sp/news/newslist.php/<?=$getnewsdir2[$j][id]?>/.html">
                <h3><?=$getnewsdir2[$j][name]?></h3>
                <p><?=$getnewsdir2[$j][intro]?></p>
                <p class="ui-li-aside">Total <strong><?php echo getnewsnum(3,$getnewsdir2[$j][id])?></strong> news</p>
            </a>
            </li>
        <?php }?>  
    ...
    <?php if($getallpagesetinfo[$i][id]==37){?>
        <ul data-role="listview" data-filter-theme="c" data-divider-theme="d" >
			<?php
            $getallpagesetinfo=getallpagesetinfo(3,'3','title,id_pageset as id,intro');//取所有版面3西班牙语 （2只显示首页 1所有页）  0关闭 3只在内页
            for($i=0;$i<sizeof($getallpagesetinfo);$i++){
                $flienameurl='/sp/about/about.php/'.$getallpagesetinfo[$i][id].'/.html';//默认关于我们
            ?>    	
