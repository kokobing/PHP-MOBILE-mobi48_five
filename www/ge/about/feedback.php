...
<div id="contentbox">
	  ...
	  #feedback .ui-body-a, .ui-overlay-a {color:#404040 /*{a-body-color}*/;background: transparent none;}
        ...
        <div id="feedback">
          <div class="feedbackline">Name *：&nbsp;&nbsp;&nbsp;&nbsp;<input id="name" type="text"  ></div>
          <div class="feedbackline">Telefon：&nbsp;&nbsp;&nbsp;&nbsp;<input id="tel" type="text" ></div>
          <div class="feedbackline">E-Mail *：&nbsp;&nbsp;&nbsp;&nbsp;<input id="email" type="text"  ></div>
          <div class="feedbackline">Nachricht *：&nbsp;&nbsp;&nbsp;&nbsp;<textarea id="content"  ></textarea></div>
          <div id="backmenu"><a onClick="sendmsg()"  href="javascript:void(0);">senden</a></div>
...
function sendmsg() {
	                    if($.trim($("#name").val())==''){alert('Bitte geben Sie Ihren Namen');return false;}
						if($.trim($("#email").val())=='' || !$("#email").val().match( /^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/ )){alert('Bitte geben Sie eine gültige E-Mail-Adresse');return false;}
						if($.trim($("#content").val())==''){alert('Bitte geben Sie Ihre Nachricht');return false;}	
						
						
						$.post('/ge/about/ajax_feedback.php',{tel:$("#tel").val(),content:$("#content").val(),name:$("#name").val(),email:$("#email").val()},function(data) {
