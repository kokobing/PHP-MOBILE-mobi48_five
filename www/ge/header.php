...
<div id="headertop">
      <div id="headerlogo"><a href="/ge/"><img src="/upload/layout/<?=$logopic?>"></a></div>
...
<div id="qrcode" data-role="popup" class="ui-content" style="max-width:280px">
        ...
        <ul data-role="listview" >
            ...
            <li data-icon="arrow-r"><a href="/ge/" style="font-size:14px;">Zuhause</a></li>
                <?php
    $getallpagesetinfo=getallpagesetinfo(2,'2','title,id_pageset as id');//取所有版面2德语 （2只显示首页 1所有页）  0关闭 3只在内页
	for($i=0;$i<sizeof($getallpagesetinfo);$i++){
		$flienameurl='/ge/about/about.php/'.$getallpagesetinfo[$i][id].'/.html';//默认关于我们
		if($getallpagesetinfo[$i][id]==32){$flienameurl='/ge/news/newsindex.php';}//新闻资讯
		if($getallpagesetinfo[$i][id]==33){$flienameurl='/ge/product/productindex.php';}//产品家族
		if($getallpagesetinfo[$i][id]==34){$flienameurl='/ge/product/producthot.php';}//热卖手机
	?>    	
       <li data-icon="arrow-r"><a href="<?=$flienameurl;?>" style="font-size:14px;"><?=$getallpagesetinfo[$i][title]?></a></li>
     <? }?>  

            <li data-icon="delete"><a href="#" data-rel="close" style="font-size:14px;">schließen</a></li>
