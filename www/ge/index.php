...
<div data-role="collapsible-set" data-theme="c" data-content-theme="d" data-inset="true" style=" margin:15px;">
    ...
    <?php
    $getallpagesetinfo=getallpagesetinfo(2,'2','title,id_pageset as id,intro');//取所有版面2德语 （2只显示首页 1所有页）  0关闭 3只在内页
	for($i=0;$i<sizeof($getallpagesetinfo);$i++){
	?>    	

 
    <div data-role="collapsible" <?php if($getallpagesetinfo[$i][id]==33){?>data-collapsed="false" <?php }?>>
    <h2><?=$getallpagesetinfo[$i][title]?></h2>

    <?php if($getallpagesetinfo[$i][id]==34){?>
            <ul data-role="listview">
            <?php 
			$newproducts=getconnewproduct('5','2','ordernum','home');//前5条 2德语
			for($j=0;$j<sizeof($newproducts);$j++){?>
                <li><a href="/ge/product/productview.php/<?=$newproducts[$j][pdir2]?>/<?=$newproducts[$j][id_prodinfo]?>/.html">
    ...
    <?php if($getallpagesetinfo[$i][id]==33){
	$getproductdir2=getproductdir2(15);
    ?>
    ...
    <?php if($getallpagesetinfo[$i][id]==32){?>
    ...
    <?php if($getallpagesetinfo[$i][id]==31){?>
        <ul data-role="listview" data-filter-theme="c" data-divider-theme="d" >
			<?php
            $getallpagesetinfo=getallpagesetinfo(2,'3','title,id_pageset as id,intro');//取所有版面2德语 （2只显示首页 1所有页）  0关闭 3只在内页
            for($i=0;$i<sizeof($getallpagesetinfo);$i++){
                $flienameurl='/ge/about/about.php/'.$getallpagesetinfo[$i][id].'/.html';//默认关于我们
            ?>    	
